...
**Status:** Planned
**Date:** 2026-08-16
**Scope:** omniagent (subtasks plugin + core wiring), omni-stack (config/prompt/templates)
...
1. **`manage_subtasks` tool does not exist.** The core enforcement
   (src/agent/main_loop.rs:912, 1615, 1631), README, and
   ...
   `get_subtask_counts`. Any agent following the instructions gets a
   tool-not-found error.
2. **Write tools disabled in allowed_tools.** Profile config.json allows only
   `subtasks_list-subtasks` (35 allowed tools total). The agent physically
...
3. **Auto-create is dead code.** main_loop.rs:282 parses the plan with
   ...
   created. Live DB has **0 rows** in `thread_subtasks`.
4. **Prompt never references subtasks.** The tool-categorizer
   (prompt_builder.rs:45) checks `tool_names.starts_with("manage_subtask")`
   but real names are `subtasks_*` → `has_subtasks` always false → subtasks
   never appear in tool lists, capability descriptions, or TOOL_GUIDANCE.
5. **Enforcement is a landmine.** If subtasks DO exist (manual DB insert), the
   end-of-thread gate orders the agent to call the phantom tool, retries
   `max_unfinished_subtask_retries` (default 3), then **force-fails the
   thread**; response_handler.rs:321 force-fails completed threads with
   unfinished subtasks too. A completed task with stale subtask rows would be
   marked failed.
6. **Cache-hostile injections.** "[Subtask Required]" (loop exit) and
   "[Progress Check]" (every 3 rounds) system messages mutate the context
   prefix mid-run, breaking DeepSeek prefix caching — the exact failure class
   fixed in Reference/DeepSeek-Prefix-Cache.md.
