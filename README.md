...
10. Sets `MATTERMOST_CHANNEL_IDS` in `.env`
...
| `MATTERMOST_CHANNEL_IDS` | Comma-separated channel IDs to watch |
...
4. **Add the channel ID to the watch list** — update `MATTERMOST_CHANNEL_IDS` in `.env`:
   ```
   MATTERMOST_CHANNEL_IDS=<existing_ids>,<new_channel_id>
   ```
   Then restart the omniagent or mattermost plugin.
