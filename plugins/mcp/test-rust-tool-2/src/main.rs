...
async fn main() -> Result<()> {
    ...
    tracing::info!("test-rust-tool-2 MCP server starting");
    ...
    tracing::info!("test-rust-tool-2 MCP server shutting down (stdin closed)");
    Ok(())
}
...
async fn handle_initialize<W: AsyncWriteExt + Unpin>(
    ...
    let result = InitializeResult {
        ...
        server_info: Implementation {
            name: "test-rust-tool-2".to_string(),
    ...
    tracing::info!("Initialized: test-rust-tool-2 v0.1.0");
    Ok(())
}
