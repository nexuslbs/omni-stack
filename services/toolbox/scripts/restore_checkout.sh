...
DATE="${1:-}"
if [ -z "$DATE" ]; then
  echo "[restore] Usage: restore_checkout YYYYMMDD"
  exit 1
fi

S3_PREFIX="${S3_PATH:-omni}/checkout/${DATE}"
...
echo "[restore] Starting restore from checkout ${DATE} (${SRC}/)"
...
echo "[restore] Step 6/7: Restoring Grafana..."
...
rm -f "$RCLONE_CONF"
echo ""
echo "[restore] Restore from checkout ${DATE} complete."
